...
internal class HentaiRead(context: MangaLoaderContext) :
		...
		val url = buildString {
			...
			when {
				...
				// Default
					...
					// HentaiRead's latest/updated catalogue lives under /hentai.
					// Keeping pagination on that route is important: /page/N at the
					// site root can resolve to a different WordPress archive (or repeat
					// the first page), which makes the app's infinite scroll stop.
					append("/hentai")
					if (page > 1) {
						append("/page/$page")
					}
	...
	override suspend fun getDetails(manga: Manga): Manga {
		...
		val title = doc.select(selectTitle).text().cleanupTitle()
		...
		val uploadDate = dateFormat.parseSafe(uploadDateString)
		val chapters = buildNumberedSeriesChapters(
			current = manga.copy(title = title),
			related = parseMangaList(doc),
	...
	private fun buildNumberedSeriesChapters(
		current: Manga,
		related: List<Manga>,
		uploadDate: Long,
	): List<MangaChapter> {
		val fallback = current.toChapter(number = 0, uploadDate = uploadDate)
		val currentTitle = current.title.toNumberedSeriesTitle() ?: return listOf(fallback)
		if (currentTitle.number != 1) {
			return listOf(fallback)
		}
		val candidates = (listOf(current) + related)
			.mapNotNull { candidate ->
				val numberedTitle = candidate.title.toNumberedSeriesTitle() ?: return@mapNotNull null
				if (numberedTitle.base != currentTitle.base) {
					return@mapNotNull null
				}
				NumberedSeriesCandidate(numberedTitle.number, candidate)
			}
			.distinctBy { it.number }
			.sortedBy { it.number }
		if (candidates.firstOrNull()?.manga?.id != current.id) {
			// The first entry is the canonical series entry and is the only one that exposes
			// the virtual chapters.
			return listOf(fallback)
		}
		val consecutive = candidates.takeConsecutiveParts(currentTitle.number)
		if (consecutive.size < 2) {
			return listOf(fallback)
		}
		return consecutive.map { candidate ->
			candidate.manga.toChapter(
				number = candidate.number,
				uploadDate = if (candidate.manga.id == current.id) uploadDate else 0L,
			)
		}
	}

	private fun Manga.toChapter(number: Int, uploadDate: Long) = MangaChapter(
		id = id,
		title = null,
	...
	override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
		// The gallery grid exposes only hencover thumbnails. The reader route has
		// the chapter-scoped, Base64-encoded full image manifest. This mirrors
		// the maintained Keiyoushi HentaiRead extension.
		val readerUrl = "${chapter.url.toAbsoluteUrl(domain).trimEnd('/')}/english/p/1/"
		val doc = webClient.httpGet(readerUrl).parseHtml()
		val baseJson = doc.selectFirst("#single-chapter-js-extra")?.data()
			?.substringAfter("var chapterExtraData = ")
			?.substringBefore(";")
			?: return emptyList()
		val baseUrl = JSONObject(baseJson).optString("baseUrl").ifBlank { return emptyList() }
		val encoded = Regex("'((?:ey)[^']+)'")
			.find(doc.selectFirst("#single-chapter-js-before")?.data().orEmpty())
			?.groupValues?.getOrNull(1)
			?: return emptyList()
		val payload = JSONObject(String(Base64.getDecoder().decode(encoded)))
		val images = payload.getJSONObject("data").getJSONObject("chapter").getJSONArray("images")
		return (0 until images.length()).map { index ->
			val url = "$baseUrl/${images.getJSONObject(index).getString("src")}" 
...
private data class NumberedSeriesCandidate(
	val number: Int,
	val manga: Manga,
)

internal data class NumberedSeriesTitle(
	val base: String,
	val number: Int,
)

internal fun String.toNumberedSeriesTitle(): NumberedSeriesTitle? {
	val match = NUMBERED_SERIES_SUFFIX.matchEntire(trim()) ?: return null
	val number = match.groupValues[2].toIntOrNull()?.takeIf { it in 1..99 } ?: return null
	val base = match.groupValues[1]
		...
		.takeIf { it.length >= 3 }
		?: return null
	return NumberedSeriesTitle(base, number)
}

private fun List<NumberedSeriesCandidate>.takeConsecutiveParts(
	firstNumber: Int,
): List<NumberedSeriesCandidate> {
	val result = ArrayList<NumberedSeriesCandidate>(size)
...
private val NUMBERED_SERIES_SUFFIX = Regex(
	pattern = """^(.+?)(?:\s*[-–—:]\s*)?(?:(?:part|chapter|ch\.?|episode|ep\.?|volume|vol\.?)\s*)?#?\s*(\d{1,3})\s*$""",
