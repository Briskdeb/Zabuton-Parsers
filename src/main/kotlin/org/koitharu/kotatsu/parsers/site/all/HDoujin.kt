...
@MangaSourceParser("HDOUJIN", "HDoujin.org", "en", type = ContentType.HENTAI)
internal class HDoujin(context: MangaLoaderContext) :
	PagedMangaParser(context, MangaParserSource.HDOUJIN, 24) {

	override val configKeyDomain = ConfigKey.Domain("hdoujin.org")
	private val apiSuffix = "api.hdoujin.org"
	...
	override suspend fun getListPage(page: Int, order: SortOrder, filter: MangaListFilter): List<Manga> {
		val isPopularWeek = order == SortOrder.POPULARITY_WEEK
		val baseUrl = if (isPopularWeek) "https://$apiSuffix/books/popular" else "https://$apiSuffix/books"
		val url = buildString {
			append(baseUrl)
			...
			if (!filter.query.isNullOrEmpty() && filter.query.startsWith("id:")) {
				...
				val response = webClient.httpGet("https://$apiSuffix/books/detail/$ipk").parseJson()
			...
			if (!isPopularWeek) {
				val sortValue = when (order) {
					...
					SortOrder.POPULARITY_FAVORITES -> "9"
			...
			if (!filter.author.isNullOrEmpty()) {
				...
				if (authorId != null) {
					val parts = authorId.split(':', limit = 2)
					if (parts.size == 2) {
						terms.add("artist:\"${parts[1]}\"")
					}
				} else {
					terms.add("artist:\"${filter.author}\"")
				}
			}

			filter.tags
				.groupBy { tag -> tag.key.removePrefix("-").substringBefore(':', "0") }
				.forEach { (namespaceValue, tags) ->
					val prefix = when (namespaceValue.toIntOrNull() ?: 0) {
						1 -> "artist"
						2 -> "circle"
						3 -> "parody"
						5 -> "character"
					...
					val values = tags.mapNotNull { tag ->
						val excluded = tag.key.startsWith("-")
						val name = tag.key.removePrefix("-").substringAfter(':', "").trim()
						name.takeIf { it.isNotEmpty() }?.let { if (excluded) "-$it" else it }
					}
					if (values.isNotEmpty()) {
						terms.add("$prefix:\"${values.joinToString(",")}\"")
					}
				}

			// Lock HDoujin searches and listing to English language
			terms.add("language:\"^english\"")
	...
	override suspend fun getPages(chapter: MangaChapter): List<MangaPage> {
		...
		for (i in 0 until entries.length()) {
			...
			// HDoujin's image CDN serves the requested derivative through `w`.
			// Without it, the API's selected quality can be ignored and the reader
			// may receive the site's fallback resolution instead.
			val fullImageUrl = "$base$imagePath?w=$selectedQuality"
	...
	private suspend fun fetchTags(): Set<MangaTag> {
		...
		val url = "https://$apiSuffix/books/tags/filters?crt=$clearance"
		return webClient.httpGet(
			url = url.toHttpUrl(),
			extraHeaders = getRequestHeaders()
		).parseJsonArray().mapJSONNotNullToSet {
			val name = it.getStringOrNull("name") ?: return@mapJSONNotNullToSet null
			val tagNamespace = it.getIntOrDefault("namespace", 0)
			MangaTag(
				title = name.toTitleCase(sourceLocale),
				key = "$tagNamespace:$name",
				source = source,
			)
		}
	}

	private suspend fun fetchAuthorsIds(): Map<String, String> = fetchTags()
		.filter { it.key.startsWith("1:") }
		.associate { it.title.lowercase() to it.key }

	private suspend fun getClearance(chapterUrl: String = "https://$domain/"): String = WebViewHelper(context)
